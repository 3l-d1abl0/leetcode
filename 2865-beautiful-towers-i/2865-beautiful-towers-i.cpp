...
public:
    ...
    long long method1(vector<int> &maxHeights){
        ...
        for(int i=0; i<N; i++){
            ...
            maxSum = max(maxSum, getSum(i, maxHeights));
            //cout<<maxSum<<"\n";
        }
    ...
    long long getSum(int peakIdx, vector<int> &maxHeights){
        
        long long sum = maxHeights[peakIdx];
        long long currMax = maxHeights[peakIdx];
        
        //go till 0
        for(int i=peakIdx-1; i >=0 ; i--){
            
            if(maxHeights[i] < currMax){
